...
def course_importance(
    ...
    assert not (
        course_number is None and course_title is None
    ), "At least course title or number should be given."
    has_run = False
    if course_number is not None:
        ...
        is_general = _in_number(course_number, general_1) or _in_number(
            course_number, general_2
        )
    ...
    if course_title is not None and not has_run:
        ...
        is_general = _in_title(course_title, general_1) or _in_title(
            course_title, general_2
        )
...
if __name__ == "__main__":
    ...
    current_courses = pd.read_csv(
        "current_courses_oct22.csv", sep="\t", dtype={0: str, 1: str, 2: str, 3: float}
    )
