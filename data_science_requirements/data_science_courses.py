...
def alternative_courses(current, core, courselist, num_points=None, time_period=None):
    ...
    core_commons = pd.merge(
        current, core["title"].reset_index(), how="inner"
    ).set_index("index")
    courselist_commons = pd.merge(
        current, courselist["title"].reset_index(), how="inner"
    ).set_index("index")
    ...
    # is_fall = ["Spring" in time for time in courselist_alts["timetable_group"]]
